...
Политика хранения снапшотов и резервных копий может различаться.
...
и раздела данных /var/lib/ для текущей машины на отдельный
...
данных в /var/lib/.snapshots/, их резервные копии в /mnt/backup/.
...
(/.snapshots/rootfs.20210324T1618/,
/var/lib/.snapshots/lib.20210324T1618/) и снапшоты были
...
``` bash
echo -e 'command="/var/lib/bin/ssh_filter_btrbk.sh --source --delete
...
-   host1.example.org - сетевое имя машины с которой будут делаться
    резервные копии
-   /, /var/lib/, /var/lib/lxc/lib/rootfs/
...
Для запуска цикла только для /var/lib/ с хоста
